...
FunctionOne.AA_composition('Positive_Test.txt',modified_lines_1)
FunctionOne.dipeptide_composition('Positive_Test.txt',modified_lines_2)
FunctionOne.Character_select('Positive_Test.txt',modified_lines_3,"1")
with open('RF_ProteinCharacterOfPositiveTest.txt','w',encoding='utf-8') as f:
    for column1, column2, column3 in zip(modified_lines_1, modified_lines_2, modified_lines_3):
        f.write(f"{column1},{column2},{column3}\n")
#湿实验阴性
modified_lines_1 = []; modified_lines_2 = []; modified_lines_3 = []
FunctionOne.AA_composition('Negative_Test.txt',modified_lines_1)
FunctionOne.dipeptide_composition('Negative_Test.txt',modified_lines_2)
FunctionOne.Character_select('Negative_Test.txt',modified_lines_3,"0")
with open('RF_ProteinCharacterOfNegativeTest.txt','w',encoding='utf-8') as f:
    for column1, column2, column3 in zip(modified_lines_1, modified_lines_2, modified_lines_3):
        f.write(f"{column1},{column2},{column3}\n")
data_P = np.loadtxt('RF_ProteinCharacterOfPositiveTest.txt',delimiter = ',',dtype = float)
data_N = np.loadtxt('RF_ProteinCharacterOfNegativeTest.txt',delimiter = ',',dtype = float)


data1 = data_P[:,0:20]
data2 = data_P[:,420:564]
data3 = data_P[:,708:]
print(data1.shape,data2.shape,data3.shape)
data = np.concatenate((data1, data2, data3), axis=1)
X1,y1 = np.split(data,(164,),axis=1)
data1 = data_N[:,0:20]
data2 = data_N[:,420:564]
data3 = data_N[:,708:]
print(data1.shape,data2.shape,data3.shape)
data = np.concatenate((data1, data2, data3), axis=1)
X2,y2 = np.split(data,(164,),axis=1)
X = np.concatenate((X1, X2), axis=0) 
y_lable = np.concatenate((y1, y2), axis=0)
...
#读取数据
FunctionTwo.deal_folder('PositiveResult_test')
FunctionTwo.deal_folder('NegativeResult_test')
positive_data, positive_labels = FunctionTwo.read_folder('PositiveResult_test', 1)  
negative_data, negative_labels = FunctionTwo.read_folder('NegativeResult_test', 0)  
print("完成了数据的读取")
#合并数据  
...
#用Transformer模型预测
positive_data, positive_labels = read_files('Positive_Test.txt','1')  
negative_data, negative_labels = read_files('Negative_Test.txt','0') 
...
print(result)
print(result.shape)
np.savetxt('Stacking_test.txt', result, delimiter=',') 

#############stacking 模型部分#####################
data = np.loadtxt('Stacking_test.txt',delimiter = ',',dtype = float)
print(data.shape)
...
np.savetxt('result_stacking.txt',y_pred_1, delimiter='\n')
#检测
